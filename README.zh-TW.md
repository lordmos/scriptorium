...
**第二步**：用 AI 程式設計助手（Claude Code / OpenCode / Cursor 等）開啟目錄，然後說這一句話：

```
[項目名] 的源碼在 [目錄路徑]。請讀 QUICK_START.md，然後向我提問。
沒有問題就開始你的工作。
```

AI 會讀取 [`QUICK_START.md`](QUICK_START.md)，向你確認書名、讀者等基本資訊，然後**自主運行全部五個階段**，完成後交付 `output/book-final.md`。

你只需要做三件事：① 回答 AI 的初始提問 → ② 確認 Phase 1 生成的大綱 → ③ 閱讀最終書稿。
...
> 💡 **並行優化**：Step 3 的三位審查員可同時工作（數據依賴不同）；Step 4 的三位讀者評審同樣可並行。多章之間也可分批並行（如每批 3 章）。
...
| 目標源碼語言 | 熟悉目標開源項目所用的編程語言（如 TypeScript、Java、Go） | ⭐⭐⭐ 必需 |
...
|------------------|---------|------|
| [Angular 原始碼深度解析（示例）](examples/) | [angular/angular](https://github.com/angular/angular) | [@lordmos](https://github.com/lordmos) |
