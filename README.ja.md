...
> 💡 **並行最適化**：Step 3の3人のレビュアーは同時に作業できます（データ依存性が異なるため）；Step 4の3人の読者評価も並行可能です。複数章も分割バッチで並行処理できます（例：{{並行バッチ数}}章ごとのバッチ）。
...
| 対象ソースコードの言語 | 対象オープンソースプロジェクトが使用するプログラミング言語の習熟（例：{{プログラミング言語}}） | ⭐⭐⭐ 必須 |
