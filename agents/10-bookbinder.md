...
- **现象**：生成的 EPUB 中 Mermaid 图表文字为白色，在浅色背景下完全不可见
- **根因**：`mmdc` 未指定 `--theme` 时会跟随系统深色模式，自动启用暗色主题（白色文字）
- **修复**：mmdc 命令必须显式指定 `--theme default --backgroundColor "#FFFFF0"`（或当前书籍背景色）
...
2. **Mermaid 图表渲染**：
   ...
   - EPUB模式：调用 `mmdc --theme default --backgroundColor "#FFFFF0"` 预渲染为 SVG；**禁止省略 `--theme default`**，否则系统深色模式会导致白色文字不可见；未安装 mmdc 时优雅降级为代码块
