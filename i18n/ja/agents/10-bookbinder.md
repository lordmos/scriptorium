...
| 状況 | 処理方法 |
|------|----------|
| `mmdc`（Mermaid CLI）がインストール済み | `mmdc -i input.mmd -o output.svg --theme default --backgroundColor "#FFFFF0"` を呼び出して事前レンダリング |
...
> ⚠️ **`--theme default` を必ず指定すること**：mmdc はシステムのダークモードを自動検出します。ダークモード時は自動的にダークテーマ（白文字）が使用され、明るいEPUB背景上で文字が見えなくなります。常に `--theme default`（または `neutral`）を指定し、`--backgroundColor` に書籍の背景色（例：`"#FFFFF0"`）を設定してください。
...
- **症状**：生成されたEPUBのMermaidダイアグラムの文字が白色で、明るい背景上で完全に見えない
- **原因**：`--theme` 未指定時、mmdc はOS のダークモードを引き継ぎ、ダークテーマ（白文字）を使用する
- **修正**：必ず `--theme default --backgroundColor "#FFFFF0"`（または書籍のBG色）を指定する
...
- [ ] （EPUBモード）MermaidダイアグラムがSVGとして事前レンダリング済み（`--theme default` で濃色文字、白文字でないこと）、またはコードブロックとして適切にフォールバックしている
...
2. **Mermaidダイアグラムのレンダリング**：
   ...
   - EPUBモード：`mmdc --theme default --backgroundColor "#FFFFF0"` で事前レンダリング；**`--theme default` の省略禁止**（システムダークモードで白文字になり不可視になる）；mmdc 未インストール時はコードブロックとしてフォールバック
