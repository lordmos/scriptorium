...
| 情境 | 處理方式 |
|------|----------|
| 系統已安裝 `mmdc`（Mermaid CLI） | 使用 `-c config.json`（`theme: 'base'` + 完整 `themeVariables`）預渲染（見坑 1） |
| 未安裝 `mmdc` | 將 Mermaid 代碼塊以 `<pre class="mermaid-source">` 形式保留，並添加提示注釋 |

> 建議：如需生成 EPUB，提前全局安裝 `npm install -g @mermaid-js/mermaid-cli`

> ⚠️ **Mermaid 配置文件中必須使用 `theme: 'base'`**：`theme: 'default'` 有 CSS 層叠，深色模式下可能造成對比度不足或白色文字。`base` 主題完全由 `themeVariables` 控制（見坑 1）。
...
- **現象**：生成的 EPUB 中 Mermaid 圖表文字爲白色不可見，或文字與節點背景色極為相近無法辨識
- **根因一**：`theme: 'default'` 有自身 CSS 變數層叠，headless Chrome 的深色模式仍可能部分覆蓋 `themeVariables`
- **根因二**：未顯式設定的變數（如 `secondaryColor`）會 fallback 到 `default` 主題預設值，可能與 `primaryTextColor` 對比度極低
- **修復**：改用 `theme: 'base'`（而非 `'default'`）。`base` 主題完全由 `themeVariables` 控制，不繼承任何 CSS 層叠，專為程序化渲染設計

```js
// ✓ 正確：使用 theme:'base' + 完整的高對比度 themeVariables
const cfgFile = path.join(tmpDir, 'mmd-config.json');
fs.writeFileSync(cfgFile, JSON.stringify({
  theme: 'base',          // 關鍵：不繼承 CSS — 一切由 themeVariables 控制
  themeVariables: {
    background:           THEME.pageBg,
    primaryColor:         '#C8E6FA',   // 淺藍填充 — 與深色文字對比鮮明
    primaryTextColor:     '#111111',   // 近黑色文字 — 最大對比度
...
// ✗ 錯誤：theme:'default' 有 CSS 層叠，對比度無法保證
...
- [ ] （EPUB模式）Mermaid 圖表已以 `-c config.json`（`theme: 'base'` + `themeVariables`）預渲染爲 SVG，或以代碼形式優雅降級
...
2. **Mermaid 圖表渲染**：
   ...
   - EPUB模式：使用 `-c config.json` 傳入 `theme: 'base'` 及完整 `themeVariables` 的 JSON 配置文件預渲染爲 SVG；`base` 主題無 CSS 層叠，對比度有保障；未安裝 mmdc 時優雅降級爲代碼塊
