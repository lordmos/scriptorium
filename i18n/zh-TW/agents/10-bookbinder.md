...
| 情境 | 處理方式 |
|------|----------|
| 系統已安裝 `mmdc`（Mermaid CLI） | 使用 `-c config.json` 指定明確的 `themeVariables` 預渲染（見坑 1） |
| 未安裝 `mmdc` | 將 Mermaid 代碼塊以 `<pre class="mermaid-source">` 形式保留，並添加提示注釋 |

> 建議：如需生成 EPUB，提前全局安裝 `npm install -g @mermaid-js/mermaid-cli`

> ⚠️ **mmdc ≥ 10.x 中僅用 `--theme default` 不夠**：headless Chrome 仍可能套用深色模式 CSS 變數，造成白色文字。請務必以 JSON 配置文件明確指定 `primaryTextColor`（見坑 1）。
...
- **現象**：生成的 EPUB 中 Mermaid 圖表文字爲白色不可見；節點背景色幾近全白
- **根因**：mmdc ≥ 10.x 中，即使指定 `--theme default`，headless Chrome 也可能套用深色模式 CSS 變數，使 `primaryTextColor` 變爲白色
- **修復**：使用 `-c config.json` 傳入含 `themeVariables` 的 JSON 配置文件，不可僅依賴 `--theme default`

```js
// ✓ 正確：以配置文件明確指定 themeVariables
const cfgFile = path.join(tmpDir, 'mmd-config.json');
fs.writeFileSync(cfgFile, JSON.stringify({
  theme: 'default',
  themeVariables: {
    background:          THEME.pageBg,    // SVG 畫布背景
    primaryColor:        '#E8E4FF',        // 節點填充色（淺紫，可見）
    primaryTextColor:    THEME.textColor,  // 節點內文字色（關鍵！）
...
// ✗ 錯誤：--theme default 在深色模式機器上仍會產生白色文字（mmdc ≥ 10）
...
- [ ] （EPUB模式）Mermaid 圖表已以 `-c config.json`（含 `themeVariables`）預渲染爲 SVG，或以代碼形式優雅降級
...
2. **Mermaid 圖表渲染**：
   ...
   - EPUB模式：使用 `-c config.json` 傳入含 `themeVariables`（尤其 `primaryTextColor`）的 JSON 配置文件預渲染爲 SVG；**`--theme default` 單獨在 mmdc ≥ 10.x 深色模式下無效**（仍會產生白色文字）；未安裝 mmdc 時優雅降級爲代碼塊
